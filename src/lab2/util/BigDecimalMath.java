...
/**
 * BigDecimal special functions. <a href="http://arxiv.org/abs/0908.3030">A Java
 * Math.BigDecimal Implementation of Core Mathematical Functions</a>
 * 
 * @since 2009-05-22
...
public class BigDecimalMath {
	...
	static public BigDecimal sqrt(final BigDecimal x, final MathContext mc) {
		...
		for (;;) {
			/*
			 * s = s -(s/2-x/2s); test correction s-x/s for being smaller than
			 * the precision requested. The relative correction is 1-x/s^2,
			 * (actually half of this, which we use for a little bit of
			 * additional protection).
			 */
			if (Math.abs(BigDecimal.ONE.subtract(x.divide(s.pow(2, locmc), locmc)).doubleValue()) < eps) break;
			s = s.add(x.divide(s, locmc)).divide(half, locmc);
			/*
			 * debugging System.out.println("itr "+x.round(locmc).toString() +
	...
	static public BigDecimal hypot(final BigDecimal x, final BigDecimal y) {
		 ...
		 * 2*x*xerr+2*y*yerr, where the two errors are 1/2 of the ulp's. Two
		 * intermediate protectio digits.
		 */
	...
	static public BigDecimal hypot(final int n, final BigDecimal x) {
		 ...
		 * the result) is |2*x*xerr|, where the error is 1/2 of the ulp. Two
		 * intermediate protection digits. zerr is a signed value, but used only
		 * in conjunction with err2prec(), so this feature does not harm.
		 */
	 ...
	 *         "Invalid Operation" errors are thrown if catastrophic
	...
	static public BigDecimal exp(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			 ...
			 * errror in invx. This is used to define the precision of the
			 * result.
			 */
			...
			/*
			 * Push the number in the Taylor expansion down to a small value
			 * where TAYLOR_NTERM terms will do. If x<1, the n-th term is of the
			 * order x^n/n!, and equal to both the absolute and relative error
			 * of the result since the result is close to 1. The x.ulp() sets
			 * the relative and absolute error of the result, as estimated from
			 * the first Taylor term. We want x^TAYLOR_NTERM/TAYLOR_NTERM! <
			 * x.ulp, which is guaranteed if x^TAYLOR_NTERM <
			 * TAYLOR_NTERM*(TAYLOR_NTERM-1)*...*x.ulp.
			 */
			...
			if (Math.pow(xDbl, TAYLOR_NTERM) < TAYLOR_NTERM * (TAYLOR_NTERM - 1.0) * (TAYLOR_NTERM - 2.0) * xUlpDbl) {
				 ...
				 * right for each power of 10 in TAYLOR_NTERM, so the addition
				 * won't add noise beyond what's already in x.
				 */
				 ...
				 * relative error in the result equals the absolute error in the
				 * argument.
				 */
				 ...
				 * result is 10 times the relative error of the base (First
				 * order binomial expansion). This looses one digit.
				 */
				MathContext mc = new MathContext(expxby10.precision() - exSc);
				/*
				 * Rescaling the powers of 10 is done in chunks of a maximum of
				 * 8 to avoid an invalid operation response by the
				 * BigDecimal.pow library or integer overflow.
				 */
				while (exSc > 0) {
	...
	static public BigDecimal exp(final MathContext mc) {
		...
		if (mc.getPrecision() < E.precision()) return E.round(mc);
			...
			/*
			 * Instantiate a 1.0 with the requested pseudo-accuracy and delegate
			 * the computation to the public method above.
			 */
	...
	static public BigDecimal log(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0)
			 ...
			 * Abramowitz-Stegun 4.124. The absolute error is err(z)/(1+z) =
			 * err(x)/x.
			 */
			...
			for (int k = 2;; k++) {
				...
				if (k % 2 == 0) resul = resul.subtract(c);
				else resul = resul.add(c);
			 ...
			 * appearing above, we avoid the hypothetical case of endless
			 * recurrence by ensuring that r >= 2.
			 */
			r = Math.max(2, r);
			 ...
			 * error in the result equals the relative error in the input,
			 * xUlpDbl/xDbl .
			 */
	...
	static public BigDecimal powRound(final BigDecimal x, final int n) {
		 ...
		 */
		if (n == 1) return x;
		else if (n == 0) return BigDecimal.ONE;
		else {
			/*
			 * The relative error in the result is n times the relative error in
			 * the input. The estimation is slightly optimistic due to the
			 * integer rounding of the logarithm. Since the standard
			 * BigDecimal.pow can only handle positive n, we split the
			 * algorithm.
			 */
			MathContext mc = new MathContext(x.precision() - (int) Math.log10((double) (Math.abs(n))));
			if (n > 0) return x.pow(n, mc);
			else return BigDecimal.ONE.divide(x.pow(-n), mc);
		}
	...
	static public BigDecimal pochhammer(final BigDecimal x, final int n) {
		 ...
		 */
		if (n < 0) throw new ProviderException("Not implemented: pochhammer with negative index " + n);
		else if (n == 0) return BigDecimal.ONE;
		else {
			/*
	...
	static public BigDecimal multiplyRound(final BigDecimal x, final BigDecimal y) {
		 ...
		 * relative errors |err(y)/y|+|err(x)/x|
		 */
	...
	static public BigDecimal divideRound(final BigDecimal x, final BigDecimal y) {
		 ...
		 * the method above will truncate trailing zeros, which may result in a
		 * smaller apparent accuracy than starte... add missing trailing zeros
		 * now.
		 */
	 ...
	 *         of the error bar) form. The value is rounded down, and on the
	 *         pessimistic side for that reason.
	 * @since 2009-05-30
	 */
